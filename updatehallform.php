...
<?php 
	...
	$opt = "<select name='yearestablished'>";
	...
	$opt2 = "<select name='cityname'>";
		while($row = $result->fetch_assoc()){
					$opt2 .= "<option value='{$row['cityname']}'>{$row['cityname']}</option>";
				}
	$opt2 .= "</select>";
...
<div class="main">
  <h1>Classical DB</h1>
<form action = "updatehallscript.php" method = "post">
		<h2>Update a Concert Hall's Information</h2>
		<p>Hall name*: <input type="text" name= "hallname"/></p>
		<p>Year Established*: <?php echo "$opt";?></p>
  		<p>City*: <?php echo "$opt2";?></p>
		<p>*Required Field</p>
	<p>Update: 
	<select name= "column">
		<option value="hallname">Hall name</option>
